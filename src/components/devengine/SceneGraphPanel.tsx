...
import {
  ...
  Camera, Download, RotateCw,
} from 'lucide-react';
import { toast } from 'sonner';
import type { SceneListItem, ImpactWarning } from '@/lib/scene-graph/types';
...
export function SceneGraphPanel({ projectId, documents }: SceneGraphPanelProps) {
  ...
  // When scene is selected, load its content into editor
  ...
  const handleRemove = async (sceneId: string) => {
    ...
    if (sg.selectedSceneId === sceneId) {
      sg.setSelectedSceneId(null);
    }
  };
  ...
  const handleMove = async (sceneId: string, direction: 'up' | 'down') => {
    ...
    if (direction === 'up' && idx > 0) {
      ...
      await sg.move.mutateAsync({
        sceneId,
        position: {
          afterSceneId: beforeScene?.scene_id,
          beforeSceneId: sg.scenes[idx - 1].scene_id,
        },
      });
      ...
      await sg.move.mutateAsync({
        sceneId,
        position: {
          afterSceneId: afterScene.scene_id,
          beforeSceneId: nextAfter?.scene_id,
        },
      });
    }
  };
          ...
          <CardContent className="space-y-3">
            ...
            {scriptDoc && (
              <Button
                ...
                onClick={() => handleExtract(scriptDoc.id)}
                disabled={sg.extract.isPending}
              >
            ...
            <div className="space-y-2">
              ...
              <Button
                ...
                disabled={sg.extract.isPending || !pasteText.trim()}
              >
    ...
    <div className="space-y-3">
                ...
                <div className="px-1 pb-1 space-y-0.5">
                  {sg.scenes.map((scene, idx) => (
                    <SceneListRow
                      ...
                      onInsertAfter={() => {
                        setInsertPosition({ afterSceneId: scene.scene_id });
                        setShowInsertDialog(true);
                      }}
                      isFirst={idx === 0}
            ...
            {selectedScene ? (
              <Card className="border-border/50">
                <CardHeader className="px-3 py-2">
                  <div className="flex items-center justify-between">
                    <CardTitle className="text-xs">
                      Scene {selectedScene.display_number}
                    </CardTitle>
                    <div className="flex items-center gap-1">
                      ...
                      {selectedScene.latest_version && selectedScene.approval_status !== 'approved' && (
                        <Button
                          ...
                          onClick={() => sg.approve.mutateAsync(selectedScene.latest_version!.id)}
                          disabled={sg.approve.isPending}
                        >
                ...
                <CardContent className="px-3 pb-3 space-y-2">
                  ...
                  <Input
                    placeholder="Slugline (e.g. INT. OFFICE - DAY)"
                    className="h-7 text-xs font-mono"
                    value={editingSlugline}
                    onChange={e => { setEditingSlugline(e.target.value); setIsDirty(true); }}
                  />

                  <Textarea
                    placeholder="Scene content..."
                    className="text-xs min-h-[250px] font-mono leading-relaxed"
      ...
      <Dialog open={showInsertDialog} onOpenChange={setShowInsertDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-sm">Insert Scene</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <Input
              placeholder="Slugline (e.g. EXT. PARK - NIGHT)"
              className="text-xs"
              value={newSlugline}
      ...
      <Card className="border-border/50">
        ...
        <CardContent className="px-2 pb-2">
          <p className="text-[10px] text-muted-foreground">
            Modify scenes to see impact warnings here.
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card className="border-border/50">
      ...
      <CardContent className="px-3 pb-3">
        <ScrollArea className="max-h-[500px]">
          <pre className="text-[11px] whitespace-pre-wrap font-mono leading-relaxed text-foreground/90">

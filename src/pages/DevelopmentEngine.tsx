...
function ConvergenceGauge({ ci, gp, gap, status }: { ci: number; gp: number; gap: number; status: string }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-4 text-center">
      ...
      <div>
        <p className="text-xs text-muted-foreground uppercase tracking-wider">Greenlight Probability</p>
                            ...
                            {iterations.map((it, idx) => (
                              <div key={it.id} className="p-4 rounded-lg border border-border/50">
                                ...
                                <div className="grid grid-cols-3 gap-2 text-center text-sm">
                                  <div>
                                    <p className="text-muted-foreground text-xs">Creative Integrity</p>
                                    <p className="font-bold">{it.reassess_ci ?? it.ci_score ?? '—'}</p>
                                  </div>
                                  <div>
                                    <p className="text-muted-foreground text-xs">Greenlight Probability</p>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {isConverged && (
                        <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-center">
                          <p className="text-sm font-medium text-emerald-400">✓ Scores Met Threshold</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            {(activeSession.current_iteration ?? 0) >= 5
                              ? 'Late-stage patch mode may be active — check Convergence panel for surgical note-resolution guidance.'
                              : 'Check the Convergence panel for the recommended next action and any remaining issues.'}
                          </p>
                        </div>
                      )}
                    </CardContent>

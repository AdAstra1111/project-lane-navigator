...
6. ESCALATION PATH: Does the premise contain an external propulsion mechanism (external antagonist, ticking clock, escalating stakes from outside the protagonist)? A purely internal/contemplative premise without external pressure is a structural risk for commercial formats. Flag as high_impact if weak.
7. RELATIONSHIP TENSION POTENTIAL: Does the premise establish or imply at least one dramatic relationship axis between characters with opposing needs or values? Flag as high_impact if no relationship friction exists.
...
These checks prevent downstream repair loops. Evaluate them NOW at idea stage. Do NOT classify them as "later" or "next_doc".`,
  ...
  format_rules: `Evaluate as FORMAT RULES. Score rule clarity, duration alignment with canonical qualifications, platform fit, and completeness. SCOPE: Format Rules MUST contain: episode duration (min/max seconds), episode count per season, platform/distribution specs, structural episode template (hook duration, act breaks, cliffhanger position), technical production constraints (location limits, cast size per episode), vertical drama-specific rules if applicable (scroll-optimised pacing, beat density). Format Rules MUST NOT contain: season arc structure or narrative spine (belongs in Season Arc), character descriptions or arcs (belongs in Character Bible), episode-by-episode story content (belongs in Episode Grid), market or finance data (belongs in Market Sheet). Flag any narrative or character content as a BLOCKER.`,
...
// ═══════════════════════════════════════════════════════════════
// SELECTIVE REGENERATION PLANNER — shared helper
//
// Extracted from the selective_regeneration_plan action so it can be
// reused by execute_selective_regeneration without code duplication.
// Takes a supabase service-role client; returns a unified plan result.
// Always returns ok:true — callers interpret recommended_scope to decide
...
//
// Only narrative-coupling relations trigger Layer 2 propagation.
// Metadata, classification, and non-narrative relations are excluded.
//
const ALLOWED_PROPAGATION_RELATIONS = new Set([
  "opposes",
  "conflicts_with",
  "allied_with",
  "mentor_of",
...
async function computeSelectiveRegenerationPlanHelper(
  ...
  const directRiskAxes = new Map<string, string>();
  const sourceUnitRaw: Array<{ unit_key: string; axis: string; status: string; reason: string }> = [];
  ...
  // 3. Enrich source units with NDG metadata + sequencing
  ...
  const propagatedAxesMap = new Map<string, string>();
  ...
  const { data: spineLinkRows } = await (supabase as any)
  ...
  const [sceneRes, verRes] = await Promise.all([
    (supabase as any).from("scene_graph_scenes").select("id,scene_key").in("id", spineSceneIds),
    (supabase as any)
  ...
  //
  // Two-layer entity expansion. Both layers are non-fatal and advisory.
  //
  // Layer 1 — entity_link (existing):
  //   axis → anchor entity → scenes containing that anchor entity
  //
  // Layer 2 — entity_propagation (new, Installment 41):
  //   axis → anchor entity → related entities (via ALLOWED_PROPAGATION_RELATIONS, depth=1)
  //          → scenes containing those related entities
  //
  // Priority for deduplication:
  //   direct > propagated > entity_link > entity_propagation
  //
  // Safety guarantees:
  //   - Only ALLOWED_PROPAGATION_RELATIONS trigger Layer 2 (opposes/conflicts_with/allied_with/mentor_of/family_of)
  //   - Depth is always 1 — never traverse beyond directly related entities
  //   - Scenes already in higher-priority categories are excluded
  //   - entity_propagation scenes are advisory only — executor never selects them
  //   - Failure is non-fatal: both layers silently degrade on any error
  //
  const alreadyScopedIds = new Set(impactedScenes.map((s: any) => s.scene_id as string));
  const entityImpactedScenes: any[] = [];
  const entityPropagatedScenes: any[] = [];
  ...
  try {
    ...
    if (groundableAxes.length > 0) {
      ...
      if (anchorEntities.length > 0) {
        ...
        // 5b.4 — Load narrative_scene_entity_links for the full expanded entity set
        ...
        // All scene IDs touched by anchor OR allowed-related entities
        ...
        if (candidateSceneIds.length > 0) {
          // 5b.5 — Load scene metadata
          const [entitySceneRes, entityVerRes] = await Promise.all([
            (supabase as any)
              .from("scene_graph_scenes")
              .select("id,scene_key")
              .in("id", candidateSceneIds),
            (supabase as any)
              .from("scene_graph_versions")
              .select("scene_id,slugline,version_number")
              .in("scene_id", candidateSceneIds)
              .order("version_number", { ascending: false }),
          ]);
          ...
          // 5b.6 — Group entity links by scene for fast lookup
          const sceneEntityMap = new Map<string, Set<string>>();
          for (const row of ((entityLinkRows || []) as any[])) {
            if (!candidateSceneIds.includes(row.scene_id)) continue;
          ...
          for (const sceneId of candidateSceneIds) {
            ...
            if (layer1Present.length > 0) {
              ...
              if (groundingClaims.length === 0) continue;
          ...
          entityPropagatedScenes.sort((a, b) => (a.scene_key as string).localeCompare(b.scene_key));
        }
      }
    }
  } catch (entityExpErr: any) {
    console.warn("[dev-engine-v2] entity-aware expansion failed (non-fatal):", entityExpErr?.message);
    // entityImpactedScenes and entityPropagatedScenes remain [] — planner continues normally
  }
  ...
  if (directCount > 0 && hasRootAxis) {
    ...
    rationale = `Root axis [${rootHits.join(", ")}] is directly affected — all downstream axes are structurally at risk. ${directCount} scene(s) directly linked; ${impactedScenes.length - directCount} additional scene(s) via NDG propagation.`;
    ...
    rationale = `Source axes [${[...directRiskAxes.keys()].join(", ")}] have no direct scene links, but ${allScenes.length} scene(s) are indirectly affected via NDG propagation.`;
  } else {
      ...
      if (selectedOptions && Array.isArray(selectedOptions) && selectedOptions.length > 0) {
        const directives = selectedOptions.map((so: any) => {
          if (so.option_id === "__other__" && so.custom_direction) {
            return `- [USER DIRECTION] Honour this creative brief exactly — implement the intent with full craft: "${so.custom_direction}"`;
          }
  ...
  "notes_addressed": ["list of note descriptions that were addressed"]
    ...
    if (action === "selective_regeneration_plan") {
      ...
      console.log("[dev-engine-v2] selective_regeneration_plan complete", {
        ...
        entity_propagated_scene_count: plan.entity_propagated_scene_count,
      });
      ...
      return new Response(JSON.stringify({
        ...
        entity_impacted_scene_count:   plan.entity_impacted_scene_count,
        entity_propagated_scenes:      plan.entity_propagated_scenes,
        entity_propagated_scene_count: plan.entity_propagated_scene_count,
      }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "execute_selective_regeneration") {
      ...
      // userId is extracted by the auth block above serve() before this point.
      // For service-role callers (lara-db-proxy) userId may be null — allowed.
      // For authenticated users, userId must be present.
      if (!userId && actor !== "service_role") {
        return new Response(JSON.stringify({
          project_id:   projectId,
          action:       "execute_selective_regeneration",
          ok:           false,
          abort:        true,
          abort_reason: "unauthorized",
          note:         "Authentication required to execute regeneration",
        }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      // ── Step 2: Concurrency guard ─────────────────────────────────────────
      // Abort if another run is currently 'running' for this project.
      ...
      if (activeRuns && activeRuns.length > 0) {
        return new Response(JSON.stringify({
          ...
          note:         `A regeneration run is already in progress for this project (run_id: ${activeRuns[0].id})`,
        }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      // ── Step 3: NDG baseline ──────────────────────────────────────────────
      // Stage 1: derive ndg_pre_at_risk_count from narrative_units directly.
      // (Full ndg_project_graph load added in Stage 2 when it matters for validation.)
      // Count scenes in scene_spine_links whose axis_key maps to a stale/contradicted unit.
      const { data: staleUnitsRaw } = await supabase
        .from("narrative_units")
        .select("unit_key,unit_type,status")
        .eq("project_id", projectId)
        .in("status", ["contradicted", "stale"]);

      const staleAxes: string[] = ((staleUnitsRaw || []) as any[]).map((u: any) => {
        const parts = (u.unit_key as string).split("::");
        return parts.length >= 2 ? parts[parts.length - 1] : null;
      }).filter(Boolean) as string[];

      const propagatedForBaseline = computePropagatedRisk(staleAxes as SpineAxis[]);
      const allBaselineAxes = new Set([
        ...staleAxes,
        ...propagatedForBaseline.flatMap(pr => pr.downstream_axes),
      ]);

      let ndgPreAtRiskCount = 0;
      if (allBaselineAxes.size > 0) {
        const { data: baselineLinks } = await supabase
          .from("scene_spine_links")
          .select("scene_id")
          .eq("project_id", projectId)
          .in("axis_key", [...allBaselineAxes]);
        ndgPreAtRiskCount = new Set(((baselineLinks || []) as any[]).map((r: any) => r.scene_id)).size;
      }
      ...
      const ABORT_SCOPES = new Set(["no_risk", "no_scene_links", "propagated_only", "no_scene_impact"]);
      if (ABORT_SCOPES.has(plan.recommended_scope)) {
        const ABORT_REASON_MAP: Record<string, string> = {
          no_risk:        "no_risk",
          no_scene_links: "no_scene_links",
          propagated_only: "propagated_only",
          no_scene_impact: "no_scenes",
        };
        return new Response(JSON.stringify({
          project_id:   projectId,
          action:       "execute_selective_regeneration",
          ok:           false,
          abort:        true,
          abort_reason: ABORT_REASON_MAP[plan.recommended_scope] ?? "no_risk",
          note:         plan.rationale,
        }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }

      if (plan.impacted_scene_count === 0) {
        return new Response(JSON.stringify({
          project_id:   projectId,
          action:       "execute_selective_regeneration",
          ok:           false,
          ...
          note:         "No impacted scenes identified — nothing to regenerate",
        }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
      ...
      const { data: runRow, error: runInsertErr } = await supabase
        ...
        .insert({
          ...
          source_axes:           plan.direct_axes,
          recommended_scope:     plan.recommended_scope,
          target_scene_ids:      targetScenes.map((s: any) => s.scene_id),
          target_scene_count:    targetScenes.length,
          status:                "pending",
          ndg_pre_at_risk_count: ndgPreAtRiskCount,
          meta_json:             { dry_run: dryRun, stage: 1 },
        })
        .select("id")
        .single();

      if (runInsertErr || !runRow) {
        console.error("[dev-engine-v2] execute_selective_regeneration: failed to create run record", runInsertErr?.message);
        throw new Error(`Failed to create regeneration_runs record: ${runInsertErr?.message ?? "unknown"}`);
      }

      const runId = runRow.id as string;

      console.log("[dev-engine-v2] execute_selective_regeneration dry-run complete", {
        project_id:         projectId,
        run_id:             runId,
        recommended_scope:  plan.recommended_scope,
        target_scene_count: targetScenes.length,
        ndg_pre_at_risk:    ndgPreAtRiskCount,
        dry_run:            dryRun,
      });
      ...
      // ── STAGE 2/3: REAL EXECUTION — bounded batch (max 5 scenes) ────────────
      //
      // Advisory lock + atomic run insert via create_regen_run_locked RPC.
      //   Acquires pg_try_advisory_xact_lock, re-checks running rows, inserts run
      ...
      const lockResult = await supabase.rpc("create_regen_run_locked", {
        ...
        p_target_scene_ids:      batchScenes.map((s) => s.scene_id),
        p_target_scene_count:    batchScenes.length,
        p_ndg_pre_at_risk_count: ndgPreAtRiskCount,
        p_meta_json:             { dry_run: false, stage: 3, batch_size: batchScenes.length },
      });
      ...
      const completedSceneKeys:  string[] = [];
      ...
      const sourceUnitKeysForProvenance = (plan.source_units || []).map((u: any) => u.unit_key);

      // ── Step 5: Per-scene execution loop ─────────────────────────────────
      for (const scene of batchScenes) {

        // ── 5.1 Load context ──────────────────────────────────────────────
        const [latestVerRes, spineLinksRes, entityLinksRes] = await Promise.all([
          supabase
            .from("scene_graph_versions")
            .select("id, version_number, content, slugline, metadata")
        ...
        // ── 5.2 Validate context — fail this scene, continue loop ─────────
        if (latestVerRes.error || !latestVerRes.data) {
          console.warn(`[batch] Scene ${scene.scene_key}: missing latest version — skipping`);
          failedSceneIds.push(scene.scene_id);
          failedSceneKeys.push(scene.scene_key);
          continue;
        }

        const prevVersion = latestVerRes.data as {
          id: string; version_number: number; content: string;
          slugline: string | null; metadata: Record<string, unknown>;
        };

        // Load entity names
        const entityIds = ((entityLinksRes.data || []) as any[]).map((l: any) => l.entity_id);
        const { data: entityRows } = entityIds.length > 0
          ? await supabase.from("narrative_entities").select("entity_key, canonical_name, entity_type").in("id", entityIds)
          : { data: [] as any[] };

        // ── 5.3 Build bounded generation prompt ───────────────────────────
        const staleUnitsForPrompt = ((sharedStaleUnits || []) as any[]).filter((u: any) => {
          const parts = (u.unit_key as string).split("::");
          return parts.length >= 2 && parts[parts.length - 1] === scene.axis_key;
        });
...
Preserve continuity. Output ONLY the rewritten scene in screenplay format.`;
        ...
        try {
          ...
          console.warn(`[batch] Scene ${scene.scene_key}: generation failed — ${genErr?.message}`);
          failedSceneIds.push(scene.scene_id);
          failedSceneKeys.push(scene.scene_key);
          continue;
        }
        ...
        if (!generatedContent || generatedContent.length < 10) {
          console.warn(`[batch] Scene ${scene.scene_key}: generation returned empty`);
          failedSceneIds.push(scene.scene_id);
          failedSceneKeys.push(scene.scene_key);
          continue;
        }

        // ── 5.5 Insert new version row ────────────────────────────────────
        ...
        const provenanceMetadata  = {
          ...
          generated_at:         sceneGeneratedAt,
          batch_order:          scene.order,
        };
        ...
        if (verErr) {
          console.warn(`[batch] Scene ${scene.scene_key}: version insert failed — ${verErr.message}`);
          failedSceneIds.push(scene.scene_id);
          failedSceneKeys.push(scene.scene_key);
          continue;
        }
        ...
        // ── 5.6 Record success ────────────────────────────────────────────
        completedSceneIds.push(scene.scene_id);
        completedSceneKeys.push(scene.scene_key);

        console.log(`[batch] Scene ${scene.scene_key}: version ${newVersionNumber} inserted, supersedes ${prevVersion.id}`);
      }
      ...
      if (completedCount === batchN && failedCount === 0) {
      ...
      await supabase
        ...
        .update({
          status:                 finalStatus,
          completed_scene_ids:    completedSceneIds,
      ...
      return new Response(JSON.stringify({
        ...
        run_id:                  execRunId,
        status:                  finalStatus,
        completed_scene_count:   completedCount,
        failed_scene_count:      failedCount,
        completed_scene_keys:    completedSceneKeys,
...
Write the COMPLETE teleplay for Episode ${epIdx} NOW.`;
    ...
    if (action === "spine_revalidate") {
      ...
      // TIMING: This action self-invokes dev-engine-v2 twice sequentially.
      // analyze ~26s + notes ~34s (parallel Class A+B) ≈ 60s + overhead.
      // Platform idle timeout: 150s (free plan) / 400s (paid plan) — well within ceiling.
      // Class A+B inference was parallelized in the notes action, cutting notes from ~47s to ~34s.
      // No per-function timeout override exists in Supabase config.toml.

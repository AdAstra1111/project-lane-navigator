...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (existingDoc) {
      ...
      const filePath = `${userId}/${project_id}/${slugTitle}_complete-season-script_${dateStr}.md`;
      if (!filePath) throw new Error("Could not generate file_path for season script document");

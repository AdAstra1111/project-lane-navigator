...
-- Atomic RPC for applying actor promotion decisions
-- This replaces the multi-write edge function pattern with a single transactional boundary.
...
BEGIN
  ...
  ELSE
    ...
    -- ONLY overwrite actor current state if rejecting the currently approved version
    -- Otherwise, this is just a historical decision about a candidate version
    IF v_current_actor.approved_version_id IS NOT NULL
       AND v_current_actor.approved_version_id = p_actor_version_id THEN
      -- Rejecting the currently approved version = effectively revoking
      UPDATE public.ai_actors SET
        promotion_status = p_final_decision_status,
        approved_version_id = null,
        roster_ready = false,
        ...
        promotion_updated_at = now()
      WHERE id = p_actor_id;
    ELSIF v_current_actor.approved_version_id IS NOT NULL THEN
      -- Rejecting a DIFFERENT version: do NOT touch current roster state
      -- Only record the decision pointer for audit
      UPDATE public.ai_actors SET
        current_promotion_decision_id = v_decision_id,
        promotion_updated_at = now()
      WHERE id = p_actor_id;
    ELSE
      -- No current approval exists: safe to set status
      UPDATE public.ai_actors SET
        promotion_status = p_final_decision_status,
        roster_ready = false,
        current_promotion_decision_id = v_decision_id,
